...
def main():
    ...
    for epoch in range(1, epochs+1):
        ...
        # Train the model
        epoch_train_loss = epoch_step("TRAIN", model, device, train_loader, optimizer, Focal_Loss, Dice_Loss, Iou_Loss)
        epoch_train_loss /= (epoch)
        training_losses.append(epoch_train_loss)

        logger.info('Validating the model...')
        # Validate the model
        epoch_valid_loss = epoch_step("VALIDATE", model, device, valid_loader, None, Focal_Loss, Dice_Loss, Iou_Loss)
    ...
    # Test the model
    test_loss = epoch_step("TEST", model, device, test_loader, None, Focal_Loss, Dice_Loss, Iou_Loss)
    logger.info(f'\nTest Loss: {test_loss}')
